...
(function () {
    ...
    el.innerHTML = `
      ...
      <div class="hero-day-card">
        ...
        <div class="hero-day-body">
          ...
          ${i.baseP ? `<div class="hero-day-prices"><span class="hero-day-new">${formatPriceSafe(i.newP)}</span><span class="hero-day-old">${formatPriceSafe(i.baseP)}</span></div>` : (i.typeBadge ? `<div class="hero-day-prices"><span class="hero-day-badge">${i.typeBadge}</span></div>` : "")}
  ...
  function renderStoreChips() {
    ...
    el.innerHTML = Object.entries(stores).map(([id, s]) => {
      ...
      return `<button class="store-chip" data-store="${id}"><div class="store-chip-logo" style="color:${s.color}">${s.icon || "🏪"}</div><div class="store-chip-name" translate="no">${s.name}</div>${count > 0 ? `<div class="store-chip-promos">${count} promos</div>` : ""}</button>`;
    }).join("");
    el.querySelectorAll(".store-chip").forEach(btn => btn.addEventListener("click", () => {
  ...
  // ---- Helper: extract usable info from PROMOTIONS entry --------------
  function promoInfo(p) {
    const stores = (typeof STORES !== "undefined" ? STORES : {});
    const store = stores[p.chain] || { name: p.chain || "Magasin", color: "#FF6B35", icon: "🏪" };
    let product = null;
  ...
  function renderAllPromos(filterChain) {
    ...
    el.innerHTML = list.map(p => {
      ...
      const pricesHTML = info.baseP ? `<div style="display:flex; align-items:baseline; gap:8px; margin-top:8px;"><span class="ptop-price-new">${formatPriceSafe(info.newP)}</span><span class="ptop-price-old">${formatPriceSafe(info.baseP)}</span>${info.savedPct > 0 ? `<span class="ptop-saving-pill">−${info.savedPct}%</span>` : ""}</div>` : (info.typeBadge ? `<div style="margin-top:8px"><span class="ptop-saving-pill">${info.typeBadge}</span></div>` : "");
      return `<div class="promo-card ${info.isSuper ? "super" : ""}" data-promo-pid="${(p.products && p.products[0]) || ""}"><div class="promo-store"><span style="display:inline-block;width:14px;height:14px;border-radius:50%;background:${info.store.color};vertical-align:middle;margin-right:4px"></span><span translate="no">${info.store.name}</span></div><div class="promo-title">${info.emoji} ${info.name}</div>${info.desc ? `<div class="promo-desc">${info.desc}</div>` : ""}${pricesHTML}<div class="promo-badge">${info.isSuper ? "🔥 SUPER " : ""}${l.promoFor}</div></div>`;
    }).join("");
    el.querySelectorAll(".promo-card").forEach(card => card.addEventListener("click", () => {
  ...
  function renderDealCategories() {
    ...
    el.querySelectorAll(".deal-cat-pill").forEach(btn => btn.addEventListener("click", () => {
  ...
  function renderBigDeals() {
    ...
    const bigStores = (typeof BIG_STORES !== "undefined" ? BIG_STORES : {});
    const bigCats = (typeof BIG_CATEGORIES !== "undefined" ? BIG_CATEGORIES : {});
    el.innerHTML = deals.map(d => {
      ...
      const imgHtml = d.image
        ...
        : `<div class="deal-card-img-fake" style="--cat-color:${c1}; --cat-color-2:${c2}; background:linear-gradient(135deg,${c1} 0%,${c2} 100%);"><span class="deal-card-img-emoji">${d.emoji || "🎁"}</span>${d.brand ? `<span class="deal-card-img-brand">${d.brand}</span>` : ""}</div>`;
      return `<div class="deal-card" data-deal-id="${d.id}"><div class="deal-card-img">${imgHtml}${savePct > 0 ? `<span class="deal-card-discount">-${savePct}%</span>` : ""}</div><div class="deal-card-body"><div class="deal-card-store"><span class="store-dot" style="background:${store.color}"></span><span translate="no">${store.name}</span></div><div class="deal-card-name">${d.name}</div><div class="deal-card-prices"><span class="deal-card-new">₪${d.discounted_price}</span>${d.original_price > d.discounted_price ? `<span class="deal-card-old">₪${d.original_price}</span>` : ""}</div>${validDate ? `<div class="deal-card-until">⏰ Jusqu'au ${validDate}</div>` : ""}</div></div>`;
    }).join("");
    el.querySelectorAll(".deal-card").forEach(card => card.addEventListener("click", () => {
  ...
  function openBigDealModal(deal) {
    ...
    const bigStores = (typeof BIG_STORES !== "undefined" ? BIG_STORES : {});
    const bigCats = (typeof BIG_CATEGORIES !== "undefined" ? BIG_CATEGORIES : {});
    const store = bigStores[deal.store] || { name: deal.store, color: "#FF6B35", icon: "🏪" };
    const cat = bigCats[deal.category] || { color: "#FF6B35", name: deal.category };
    ...
    content.innerHTML = `<div class="modal-head"><h3>${deal.emoji || "🎁"} ${deal.name}</h3><button class="modal-close" onclick="document.getElementById('product-modal').classList.remove('open')">×</button></div><div style="padding:20px 24px;"><div style="height:180px; border-radius:16px; background:linear-gradient(135deg,${cat.color},${shade(cat.color,-20)}); display:flex; flex-direction:column; align-items:center; justify-content:center; gap:8px; margin-bottom:16px;"><span style="font-size:96px; line-height:1; filter: drop-shadow(0 4px 12px rgba(0,0,0,.2));">${deal.emoji || "🎁"}</span>${deal.brand ? `<span style="background:rgba(0,0,0,.35); color:white; font-size:12px; font-weight:800; padding:4px 12px; border-radius:999px; letter-spacing:.1em; text-transform:uppercase;">${deal.brand}</span>` : ""}</div><div class="pm-cat" style="background:${cat.color}22; color:${cat.color};">${cat.name}</div><div style="display:flex; align-items:baseline; gap:12px; margin:14px 0;"><span style="font-size:32px; font-weight:900; color:var(--brand); font-variant-numeric:tabular-nums;">₪${deal.discounted_price}</span><span style="font-size:16px; color:var(--muted); text-decoration:line-through;">₪${deal.original_price}</span>${savePct > 0 ? `<span class="ptop-saving-pill">-${savePct}%</span>` : ""}</div>${deal.description ? `<p class="pm-desc">${deal.description}</p>` : ""}<div class="pm-section-title">Disponible chez</div><div class="pm-price-row" style="cursor:default;"><div style="display:flex;align-items:center;gap:10px;"><span class="store-mini" style="background:${store.color}; min-width:32px; height:32px; font-size:14px;">${store.icon || "🏪"}</span><div><div class="pm-store-name" translate="no">${store.name}</div><div style="font-size:11px;color:var(--muted);">Économie ₪${saving.toFixed(0)}</div></div></div><button class="btn primary sm" onclick="document.getElementById('product-modal').classList.remove('open');" style="margin-left:auto;">OK</button></div>${deal.valid_until ? `<div class="setting-hint" style="margin-top:14px; text-align:center;">⏰ Offre valable jusqu'au ${new Date(deal.valid_until).toLocaleDateString("fr-FR", { day: "numeric", month: "long" })}</div>` : ""}</div>`;
  ...
  function renderFavsView() {
    ...
    el.innerHTML = favIds.slice(0, 50).map(pid => {
      ...
      const store = stores[bestStore] || { name: bestStore };
      const promo = (typeof PROMOTIONS !== "undefined") ? PROMOTIONS.find(pr => pr.product_id === pid) : null;
      return `<div class="fav-card" data-pid="${pid}"><div class="fav-card-icon">${p.icon || "📦"}</div><div class="fav-card-info"><div class="fav-card-name">${p.name}</div><div class="fav-card-prices"><span class="fav-card-best">₪${bestPrice.toFixed(2)}</span><span class="fav-card-store">${l.cheapestAt} <strong translate="no">${store.name}</strong></span></div>${promo ? `<div class="fav-card-promo">🎁 Promo active</div>` : ""}</div><button class="fav-heart" data-pid="${pid}" aria-label="Retirer">❤️</button></div>`;
    }).join("");
    el.querySelectorAll(".fav-card").forEach(card => card.addEventListener("click", (e) => {
  ...
  function renderCheapestStoreSummary() {
    ...
    if (!list || !list.cart || Object.keys(list.cart).length === 0) { el.hidden = true; return; }
    if (typeof window.computeStoreTotals !== "function") { el.hidden = true; return; }
    try {
      const totals = window.computeStoreTotals();
      if (!totals || totals.length === 0) { el.hidden = true; return; }
      const cheapest = totals[0];
      const stores = (typeof STORES !== "undefined" ? STORES : {});
      const store = stores[cheapest.store] || { name: cheapest.store };
      const next = totals[1];
      const saving = next ? (next.total - cheapest.total) : 0;
      el.hidden = false;
      el.innerHTML = `<div class="comparison-card-title">💸 Le moins cher</div><div class="comparison-card-store" translate="no">${store.icon || "🏪"} ${store.name}</div><div class="comparison-card-amount">₪${cheapest.total.toFixed(2)}</div>${saving > 0 ? `<div class="comparison-card-saving">+ Économie estimée : ₪${saving.toFixed(2)} vs ${(stores[next.store] && stores[next.store].name) || next.store}</div>` : ""}`;
  ...
  function init() {
    ...
    if (quickNearby) quickNearby.addEventListener("click", () => { if (window.openNearbyModal) window.openNearbyModal(); });
